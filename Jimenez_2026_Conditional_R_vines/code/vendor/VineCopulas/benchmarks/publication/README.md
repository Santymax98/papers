...
- `external_rvinecopulib.csv` from the R benchmark.
...
## Optional external comparison: rvinecopulib

After Julia references and `case_manifest.csv` exist:
...
The external script benchmarks `rvinecopulib::pvinecop()` on compatible
continuous D-vine CDF cases. It intentionally skips general hyperrectangles
instead of reconstructing them through `2^d` inclusion-exclusion terms.

`pvinecop()` uses quasi-Monte Carlo integration controlled by `n_mc`; the
external comparison is therefore a useful state-of-practice baseline rather
than an exact reference.
